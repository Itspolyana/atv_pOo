...
def main():
    ...
    if matchingGuitars:
        print("Erin, Olha só o que encontramos para você:")
        for guitar in matchingGuitars:
            guitarSpec = guitar.getSpec()
            print(f"\nGuitarra: {guitar.getSerialNumber()} {guitarSpec.getBuilder().value} {guitarSpec.getModel()} {guitarSpec.getTypeG().value} guitar:\n{guitarSpec.getBackWood().value} na traseira e laterais, {guitarSpec.getTopWood().value} no tampo, com {guitarSpec.getNumStrings()} cordas.\nEla pode ser sua por apenas US${guitar.getPrice():.2f}!")
    else:
        print("Sinto muito Erin, no momento não encontramos nada para você")
